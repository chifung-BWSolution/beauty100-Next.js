...
function mapSupabaseToArticle(record: any): ArticleData {
  ...
  const wordCount = body
    ...
    .reduce((acc, b) => {
      if (b.type === 'html') return acc + (b.html.replace(/<[^>]*>/g, '').length || 0);
                  ...
                  {images.map((img, i) => (
                    <button
                      ...
                      className={`relative rounded-lg overflow-hidden aspect-square transition-all ${
                        i === currentIndex
                          ? 'ring-2 ring-fuchsia-400 ring-offset-2 ring-offset-black/60 opacity-100'
          ...
          {images.map((img, i) => (
            <button
              ...
              className={`relative shrink-0 w-[72px] h-[54px] sm:w-20 sm:h-[60px] rounded-lg overflow-hidden transition-all duration-200 ${
                i === selectedIndex
                  ? 'ring-2 ring-fuchsia-400 ring-offset-1 ring-offset-white opacity-100 shadow-md'
            ...
            >
              ...
              />
              {i === selectedIndex && (
                <div className="absolute bottom-0.5 left-1/2 -translate-x-1/2 w-4 h-0.5 bg-fuchsia-400 rounded-full" />
              )}
            </button>
          ))}
        </div>
          ...
          case 'quote':
            return (
              ...
              >
                ...
                {block.author && (
                  <cite className="block mt-2 text-[14px] text-fuchsia-500 font-medium not-italic">
                    — {block.author}
                  </cite>
                )}
              </blockquote>
...
function ArticleToolbar({ onCopyLink }: { onCopyLink: () => void }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <button
        onClick={handleCopy}
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[12px] font-medium text-slate-500 hover:text-fuchsia-500 hover:bg-fuchsia-50 transition-all border border-slate-200 hover:border-current"
        title="複製連結"
      >
      ...
      <button
        ...
        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[12px] font-medium transition-all border ${
          bookmarked
            ? 'text-fuchsia-500 bg-fuchsia-50 border-current'
            : 'text-slate-500 hover:text-fuchsia-500 hover:bg-fuchsia-50 border-slate-200 hover:border-current'
        }`}
        title="收藏"
      >
...
function RelatedArticleCard({ article }: { article: RelatedArticle }) {
  return (
    <Link
      href={`/skincare/${encodeURIComponent(article.slug)}`}
    ...
    >
      <div className="relative h-40 overflow-hidden">
        ...
        <div className="absolute top-2.5 left-2.5">
          <Badge className="bg-fuchsia-500 text-white border-0 text-[14px] shadow-sm">
            {article.tag}
          </Badge>
        </div>
      </div>
      <div className="p-3.5">
        <h3 className="font-bold text-slate-800 text-[14px] mb-2 line-clamp-2 group-hover:text-fuchsia-500 transition-colors leading-snug">
          {article.title}
        </h3>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col justify-center min-w-0">
        <Badge className="bg-fuchsia-50 text-fuchsia-500 border-0 text-[12px] w-fit mb-1">
          {article.tag}
        </Badge>
        <h4 className="text-[14px] font-semibold text-slate-700 line-clamp-2 group-hover:text-fuchsia-500 transition-colors leading-snug">
          {article.title}
        </h4>
    ...
    <div className="bg-white rounded-xl border border-slate-100/80 shadow-sm p-4">
      <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2 mb-3.5">
        <span
          ...
          style={{ background: SKINCARE_THEME.accentGradient }}
        />
...
export default function SkincareArticlePage() {
  ...
  useEffect(() => {
    async function fetchArticle() {
      ...
      try {
        ...
        // Fetch related articles (same category - skincare)
        ...
        // Extract popular tags from skincare articles
        ...
        <div className="flex items-center justify-center min-h-[60vh]">
          <div className="text-center">
            ...
            <div className="flex flex-col gap-3 items-center">
              ...
              <Link href="/skincare" className="inline-flex items-center gap-1.5 text-fuchsia-500 font-medium hover:text-fuchsia-600">
  ...
  return (
    <PublicLayout activeHref="/skincare">
      ...
      >
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          <nav className="flex items-center gap-1.5 text-[14px] text-slate-400 mb-4">
            <Link href="/" className="hover:text-fuchsia-500 transition-colors">
              首頁
            </Link>
            <ChevronRight className="w-3 h-3" />
            <Link href="/skincare" className="hover:text-fuchsia-500 transition-colors">
              化妝護膚
            </Link>
            <ChevronRight className="w-3 h-3" />
            <span className="text-fuchsia-500 font-medium">{article.title.length > 20 ? article.title.substring(0, 20) + '...' : article.title}</span>
          </nav>

          {/* Category tag */}
          <Link href="/skincare">
            <Badge className="bg-fuchsia-500 text-white border-0 text-[12px] shadow-sm mb-3 hover:bg-fuchsia-600 transition-colors cursor-pointer">
              化妝護膚
            </Badge>
          </Link>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <article className="flex-1 min-w-0 max-w-[760px]">
            ...
            <div className="mt-10 pt-6 border-t border-slate-100">
              ...
              <Link
                href="/skincare"
                className="inline-flex items-center gap-1.5 text-[14px] text-fuchsia-500 font-medium hover:text-fuchsia-600 transition-colors"
              >
            ...
            <SidebarSection title="熱門文章" icon={Flame}>
              <div className="space-y-3">
                {sidebarTrending.map((sa, i) => (
                  <Link
                    key={i}
                    href={`/skincare/${encodeURIComponent(sa.slug)}`}
                    className="group flex items-start gap-3 hover:bg-fuchsia-50/40 -mx-2 px-2 py-1.5 rounded-lg transition-colors"
                  >
                    <span className="text-lg font-bold text-fuchsia-500/40 shrink-0 w-6 text-right leading-tight">
                      {String(i + 1).padStart(2, '0')}
                    </span>
                    <div className="min-w-0">
                      <h4 className="text-[14px] font-medium text-slate-700 line-clamp-2 group-hover:text-fuchsia-500 transition-colors leading-snug">
                        {sa.title}
                      </h4>
            ...
            <SidebarSection title="編輯推薦" icon={Star}>
              <div className="space-y-2.5">
                {relatedArticles.slice(0, 4).map((pick, i) => (
                  <Link
                    key={i}
                    href={`/skincare/${encodeURIComponent(pick.slug)}`}
                    className="group flex items-center gap-2.5 hover:bg-fuchsia-50/40 -mx-2 px-2 py-1.5 rounded-lg transition-colors"
                  >
                    <Bookmark className="w-3.5 h-3.5 text-fuchsia-500/60 shrink-0" />
                    <div className="min-w-0">
                      <h4 className="text-[14px] font-medium text-slate-700 line-clamp-1 group-hover:text-fuchsia-500 transition-colors">
                        {pick.title}
                      </h4>
                      <span className="text-[12px] text-fuchsia-500 font-medium">
                        {pick.tag}
                      </span>
                    </div>
                  </Link>
              ...
              <div className="space-y-2.5">
                {sidebarTrending.slice(0, 4).map((sa, i) => (
                  ...
                  >
                    ...
                    />
                    <div className="min-w-0">
                      <h4 className="text-[12px] font-medium text-slate-700 line-clamp-2 group-hover:text-fuchsia-500 transition-colors leading-snug">
                        {sa.title}
                      </h4>
          ...
          <div className="space-y-2">
            {relatedArticles.slice(0, 4).map((pick, i) => (
              ...
              >
                <Bookmark className="w-3.5 h-3.5 text-fuchsia-500/60 shrink-0" />
                <div>
                  <h4 className="text-[14px] font-medium text-slate-700 group-hover:text-fuchsia-500 transition-colors">
                    {pick.title}
                  </h4>
                  <span className="text-[12px] text-fuchsia-500 font-medium">{pick.tag}</span>
                </div>
              </Link>
            ))}
          </div>
        ...
        <div className="bg-white rounded-xl border border-slate-100/80 shadow-sm p-4">
          ...
          <div className="space-y-2.5">
            {sidebarTrending.map((sa, i) => (
              ...
              >
                <span className="text-base font-bold text-fuchsia-500/40 shrink-0 w-5 text-right leading-tight">
                  {String(i + 1).padStart(2, '0')}
                </span>
                <div>
                  <h4 className="text-[14px] font-medium text-slate-700 group-hover:text-fuchsia-500 transition-colors leading-snug">
                    {sa.title}
                  </h4>
                  <span className="text-[14px] text-slate-400 flex items-center gap-1 mt-0.5">
                    <Eye className="w-2.5 h-2.5" />
