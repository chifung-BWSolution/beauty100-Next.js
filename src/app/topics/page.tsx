...
function FeaturedMainCard({ article }: { article: Article }) {
  return (
    <Link href={`/topics/${encodeURIComponent(article.slug || '')}`} className="group relative block rounded-2xl overflow-hidden bg-slate-900 h-[320px] sm:h-[360px] lg:h-full">
      <img
        src={article.image}
        alt={article.title}
...
function FeaturedSupportCard({ article }: { article: Article }) {
  return (
    <Link href={`/topics/${encodeURIComponent(article.slug || '')}`} className="group relative block rounded-xl overflow-hidden bg-slate-900 h-[140px] sm:h-[130px] lg:h-full">
      <img
        src={article.image}
        alt={article.title}
...
export default function TopicsPage() {
  ...
  const { trendingArticles, editorPicks, latestUpdates } = useSidebarData('trending-topics');
  ...
  useEffect(() => {
    ...
    async function fetchPopularTags() {
      try {
        // Fetch tags from ALL active articles to ensure we only show tags that have real articles
            ...
            <section>
              ...
              {loading ? (
                <div className="text-center py-16 bg-white/60 rounded-xl border border-slate-100/60">
                  ...
                  <h3 className="text-base font-semibold text-slate-700 mb-1">正在載入文章...</h3>
                  <p className="text-sm text-slate-400">請稍候</p>
                </div>
                ...
                <div className="text-center py-16 bg-white/60 rounded-xl border border-slate-100/60">
                  ...
                  <p className="text-sm text-slate-400">敬請期待更多焦點話題相關文章！</p>
                </div>
              )}
            </section>
          ...
          <aside className="hidden lg:block w-[300px] shrink-0 space-y-5">
            ...
            <SidebarSection title="編輯推薦" icon={Star}>
              <div className="space-y-2.5">
                {editorPicks.slice(0, 10).map((pick, i) => (
                  <Link
                    key={i}
