...
function FeaturedMainCard({ article }: { article: Article }) {
  return (
    <Link href={`/entertainment/${encodeURIComponent(article.slug || 'entertainment')}`} className="group relative block rounded-2xl overflow-hidden bg-slate-900 h-[320px] sm:h-[360px] lg:h-full">
      <img
        src={article.image}
        alt={article.title}
...
function FeaturedSupportCard({ article }: { article: Article }) {
  return (
    <Link href={`/entertainment/${encodeURIComponent(article.slug || 'entertainment')}`} className="group relative block rounded-xl overflow-hidden bg-slate-900 h-[140px] sm:h-[130px] lg:h-full">
      <img
        src={article.image}
        alt={article.title}
...
function ArticleCard({ article }: { article: Article }) {
  return (
    <Link
      href={`/entertainment/${encodeURIComponent(article.slug || 'entertainment')}`}
...
export default function EntertainmentPage() {
  ...
  const { trendingArticles, editorPicks, latestUpdates } = useSidebarData('entertainment');
      ...
      try {
        ...
        if (data) {
          const mapped: Article[] = data.map((item: any, idx: number) => ({
            ...
            image: item.cover_image_url || 'https://images.unsplash.com/photo-1509967419530-da38b4704bc6?w=800&q=80',
            tag: item.tags?.[0] || '娛樂圈',
          ...
          const sortedTags = Object.entries(tagCounts)
            ...
            .map(([tag]) => tag)
            .slice(0, 16);
          setPopularTags(sortedTags.length > 0 ? sortedTags : ['凍齡', '素顏', '紅毯', 'K-Beauty', '明星同款', '護膚routine', '底妝', '男士護膚', '美容院', '造型師', '化妝教學', '逆齡']);
        }
      } catch (err) {
            ...
            <section>
              ...
              {loading ? (
                ...
                <div className="text-center py-16 bg-white/60 rounded-xl border border-slate-100/60">
                  ...
                  <p className="text-sm text-slate-400">敬請期待更多娛樂圈相關文章！</p>
                </div>
              )}
            </section>
