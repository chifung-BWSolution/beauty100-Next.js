...
const theme = ENTERTAINMENT_THEME;
...
function RelatedArticleCard({ article }: { article: RelatedArticle }) {
  return (
    <Link
      href={`/entertainment/${encodeURIComponent(article.slug)}`}
    ...
    >
      ...
      <div className="p-3.5">
        <h3 className="font-bold text-slate-800 text-[14px] mb-2 line-clamp-2 group-hover:text-fuchsia-600 transition-colors leading-snug">
          {article.title}
        </h3>
    ...
    <div className="bg-white rounded-xl border border-slate-100/80 shadow-sm p-4">
      <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2 mb-3.5">
        <span
          ...
          style={{ background: 'linear-gradient(180deg, #e879f9, #a21caf)' }}
        />
  ...
  useEffect(() => {
    async function fetchArticle() {
      ...
      try {
        ...
        // Fetch main article by handle (slug)
        ...
        // Sidebar trending (latest entertainment articles)
        ...
        // Extract popular tags from entertainment articles
    ...
    return (
      <PublicLayout>
        <div className="flex items-center justify-center min-h-[60vh]">
          <div className="text-center">
            <div className="w-16 h-16 mx-auto mb-4 rounded-2xl flex items-center justify-center bg-fuchsia-50">
              <ImageIcon className="w-7 h-7 text-fuchsia-300" />
            </div>
            ...
            <div className="flex flex-col gap-3 items-center">
              ...
              <Link href="/entertainment" className="inline-flex items-center gap-1.5 text-fuchsia-500 font-medium hover:text-fuchsia-600">
      ...
      >
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          <nav className="flex items-center gap-1.5 text-[14px] text-slate-400 mb-4">
            ...
            <Link href="/entertainment" className="hover:text-fuchsia-500 transition-colors">
              娛樂圈
            </Link>
            <ChevronRight className="w-3 h-3" />
            ...
            <SidebarSection title="熱門文章" icon={Flame}>
              <div className="space-y-3">
                {sidebarTrending.map((sa, i) => (
                  <Link
                    key={i}
                    href={`/entertainment/${encodeURIComponent(sa.slug)}`}
            ...
            <SidebarSection title="編輯推薦" icon={Star}>
              <div className="space-y-2.5">
                {relatedArticles.slice(0, 4).map((pick, i) => (
                  <Link
                    key={i}
                    href={`/entertainment/${encodeURIComponent(pick.slug)}`}
