...
import {
  ...
  X, ChevronLeft, ZoomIn, ImageIcon, Droplets,
} from 'lucide-react';
...
const FALLBACK_IMAGE = 'https://images.unsplash.com/photo-1544161515-4ab6ce6db874?w=800&q=80';
const FALLBACK_AVATAR = 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=100&q=80';

/* Category theme for 身體保養 */
...
const BODY_CARE_THEME: CategoryTheme = {
  badgeClass: 'bg-blue-500 text-white border-0',
  badgeHoverClass: 'hover:bg-blue-600',
  headerBg: 'linear-gradient(135deg, #eff6ff 0%, #f0f9ff 30%, #e0f2fe 70%, #eff6ff 100%)',
  accentGradient: 'linear-gradient(180deg, #60a5fa, #2563eb)',
                  ...
                  {images.map((img, i) => (
                    <button
                      ...
                      className={`relative rounded-lg overflow-hidden aspect-square transition-all ${
                        i === currentIndex
                          ? 'ring-2 ring-blue-400 ring-offset-2 ring-offset-black/60 opacity-100'
          ...
          {images.map((img, i) => (
            <button
              ...
              className={`relative shrink-0 w-[72px] h-[54px] sm:w-20 sm:h-[60px] rounded-lg overflow-hidden transition-all duration-200 ${
                i === selectedIndex
                  ? 'ring-2 ring-blue-400 ring-offset-1 ring-offset-white opacity-100 shadow-md'
    ...
    <div className="flex items-center gap-2">
      ...
      <button
        ...
        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[12px] font-medium transition-all border ${
          bookmarked
            ? `${theme.linkTextClass} ${theme.tagBgClass} border-current`
            : `text-slate-500 ${theme.hoverTextClass} hover:${theme.tagBgClass} border-slate-200 hover:border-current`
        }`}
        title="收藏"
      >
...
function RelatedArticleCard({ article, theme }: { article: RelatedArticle; theme: CategoryTheme }) {
  return (
    <Link
      href={`/body-care/${encodeURIComponent(article.slug)}`}
    ...
    >
      <div className="relative h-40 overflow-hidden">
        ...
        <div className="absolute top-2.5 left-2.5">
          <Badge className={`${theme.badgeClass} text-[14px] shadow-sm`}>
            {article.tag}
          </Badge>
        </div>
      </div>
    ...
    <div className="bg-white rounded-xl border border-slate-100/80 shadow-sm p-4">
      <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2 mb-3.5">
        <span
          ...
          style={{ background: theme.accentGradient }}
        />
        <Icon className={`w-3.5 h-3.5 ${theme.iconColorClass}`} />
        {title}
      </h3>
      {children}
    </div>
...
export default function BodyCareArticlePage() {
  ...
  useEffect(() => {
    async function fetchArticle() {
      ...
      try {
        ...
        // Fetch related articles (same category = body-care)
        ...
        // Sidebar trending (latest body-care articles)
        ...
        // Extract popular tags from body-care articles
  ...
  if (loading) {
    return (
      <PublicLayout activeHref="/body-care">
        ...
        <div className="flex items-center justify-center min-h-[60vh]">
          <div className="text-center">
            ...
            <div className="flex flex-col gap-3 items-center">
              ...
              <Link href="/body-care" className="inline-flex items-center gap-1.5 text-blue-500 font-medium hover:text-blue-600">
      ...
      >
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          <nav className="flex items-center gap-1.5 text-[14px] text-slate-400 mb-4">
            ...
            <Link href="/body-care" className={`${theme.hoverTextClass} transition-colors`}>
              身體保養
            </Link>
            <ChevronRight className="w-3 h-3" />
              ...
              <div className={`mb-8 relative rounded-2xl overflow-hidden bg-gradient-to-r ${theme.promoFromClass} border ${theme.promoBorderClass} p-5`}>
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                  <div>
                    ...
                    <h4 className="text-sm font-bold text-slate-700">探索更多身體保養專題文章</h4>
                    <p className="text-[12px] text-slate-500 mt-0.5">由編輯團隊嚴選嘅深度分析同專家觀點</p>
                  </div>
                  <Link href="/body-care" className={`shrink-0 inline-flex items-center gap-1 text-xs font-semibold ${theme.promoBtnTextClass} ${theme.promoBtnBgClass} ${theme.promoBtnHoverBgClass} px-4 py-2 rounded-full transition-colors`}>
                    瀏覽更多
            ...
            <SidebarSection title="熱門文章" icon={Flame} theme={theme}>
              <div className="space-y-3">
                {sidebarTrending.map((sa, i) => (
                  <Link
                    key={i}
                    href={`/body-care/${encodeURIComponent(sa.slug)}`}
            ...
            <SidebarSection title="編輯推薦" icon={Star} theme={theme}>
              <div className="space-y-2.5">
                {relatedArticles.slice(0, 4).map((pick, i) => (
                  <Link
                    key={i}
                    href={`/body-care/${encodeURIComponent(pick.slug)}`}
