-- Force fix: completely rebuild payouts from scratch to eliminate duplicates
-- The previous merge migration may have failed silently due to missing payout_items references

-- Step 1: NULL out FK references
UPDATE order_items SET payout_id = NULL WHERE payout_id IS NOT NULL;

-- Step 2: Clear payout_items and payouts completely
...
-- Step 4: Re-backfill with simple monthly logic (same as migration 165 but guaranteed clean)
DO $$
DECLARE
  rec RECORD;
  v_payout_id UUID;
